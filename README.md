...
- **10 大功能卡片直达**：今日任务、靶向组卷、同源变式、考纲Diff、切片入库、院校侦察、双校对标、简章监控、看板更新、公众号检索；
  卡片图标采用**内联 SVG 矢量渲染**（非 emoji 字符），跨系统字形一致、颜色跟随主题主色自动重绘；
...
- **纯终端极速体验**：数字快捷键 `1`-`9` 秒级穿透，资源占用极低；
  未安装 `textual`（或非交互式终端）时自动回落到纯文本控制台，功能不缺失；
...
4. **大模型端点要求**
 ...
 但请注意部分第三方中转站存在 **~60 秒网关响应超时**，推理型模型的长讲解会被中途掐断
 （表现为 `Remote end closed connection`）。遇到此类报错请优先换用官方端点或响应更快的模型。
5. **自命题科目需自行生成考纲**
...
# pytest 单元与进程层测试（CLI 入口、并发与原子性、主题设计系统、看板资产、SVG 图标与设置面板等，共 222 项）
python -m pytest -q
```

- **测试保障**：主套件 25 组测试集 **284 项断言** + 新功能专项 **130 项断言** + pytest 单元/进程层 **222 项**（合计 **636 项**，含可选依赖缺失时自动跳过的用例），全链路覆盖工业级 Agent Loop、权限沙箱、研招情报、多模型开放题判分等；
