...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tests-2333%20Passed-10b981?style=flat-square&logo=checkmarx&logoColor=white" alt="Tests 2333 Passed" />
...
# pytest 单元与进程层测试（CLI 入口、并发与原子性、主题设计系统、看板资产、SVG 图标与设置面板等，共 1900 项）
python -m pytest -q
```

> [!NOTE]
> **测试计数已做成环境无关，但前提仍要写清**（下列数字为 2026-09-26 实测）：
>
...
> - `pytest tests/`：共 **1900 项**（完整工作区、已配 `study_plan` 的 `ky_config.json`、
> **含** `dist/` 构建产物，实测 1897 通过 + 3 跳过）。跳过项为联网测试未设
...
- **测试保障**：主套件 26 组测试集 **305 项断言** + 新功能专项 **131 项断言** + pytest 单元/进程层 **1897 项**（合计 **2333 项通过**，另有 3 项按环境跳过），全链路覆盖工业级 Agent Loop、权限沙箱、研招情报、多模型开放题判分等；
