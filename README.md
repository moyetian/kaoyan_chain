...
**基于 AI Agent 私人教师协议、外置状态机驱动与自动化自测看板的开源考研备考工程脚手架**



![Status](https://img.shields.io/badge/Status-Template%20Ready-success.svg?style=flat-square)

![Python](https://img.shields.io/badge/Python-3.8%2B-blue.svg?style=flat-square)

![KaTeX](https://img.shields.io/badge/Math-KaTeX%20LaTeX-brightgreen.svg?style=flat-square)

![Mobile](https://img.shields.io/badge/Mobile-Responsive%20PWA-orange.svg?style=flat-square)

![Privacy](https://img.shields.io/badge/Privacy-Local%20First-green.svg?style=flat-square)
...
- **特点**：国内极高人气的多大模型桌面端，聚合 DeepSeek-V4、Claude-opus-5.0、OpenAI-GPT5.6、Kimi K2.7、Qwen3.8 等。
- **使用方法**：
...
- **全模型 API 原生接入**：直接配置 DeepSeek (V4)、智谱 GLM、通义千问、月之暗面 Kimi、OpenAI 或本地 Ollama；
...
#### 考研专有智能体技能 (Built-in Skills)


| 专有技能 (Skill)     | 指令语法                  | 核心功能与亮点                                                   |
| ---------------- | --------------------- | --------------------------------------------------------- |
| 🌐 **公式美化与实时伴侣** | `/view` 或在终端自动触发      | 终端自动美化原始 LaTeX 语法（积分/分式/极限），并在浏览器中以**印刷级 KaTeX 排版**实时对照呈现 |
| 📸 **多模态视觉批改**   | `/img <路径>` 或直接拖拽图片路径 | 自动提取手写草稿与试卷截图中的题目（LaTeX 还原），逐行核对推导步骤，标注采分点与错因五分类          |
| 📐 **数学高精度符号验算** | `/calc <数学表达式>`       | 极限、导数、定积分/不定积分、矩阵特征值与泰勒展开严谨验算，**彻底消灭 AI 计算幻觉**            |
| 🧱 **英语长难句搭积木**  | `/dissect <长难句>`      | 抽取句子主干骨架，解构从句层级与伴随修饰，标注考研高频生词，两步意译润色                      |
| 📚 **教材真题资料检索**  | `/pdf [关键词]`          | 全文检索四科「参考资料/」文件夹中的教材电子书、课后题与历年真题库                         |
| 🧩 **技能中心总览**    | `/skills`             | 打印当前环境已装载的所有技能及其就绪状态                                      |


### 3. 连接微信、QQ、钉钉、飞书聊天软件 (IM Bridge)

在终端运行 `ky config` 即可进入分类多选菜单自由配置：

- **微信 / 企业微信 / 微信 ClawBot**：选择 \[1\] 填入群机器人 Webhook 地址；
- **钉钉**：选择 \[2\] 填入钉钉群自定义机器人 Webhook 与加签 Secret；
- **飞书**：选择 \[3\] 填入飞书群自定义机器人 Webhook；
- **QQ (OneBot 11 / NapCat)**：选择 \[4\] 填入本地或服务器 OneBot HTTP 地址与目标群号。
*(填入后可当场发送一条测试消息验证连通性)*
...
### 4. 数学公式直观化与实时前端联动 (LaTeX 还原方案)

传统黑白终端在处理考研数学时，往往会输出晦涩难读的原始 LaTeX 代码（如 `\(f(x)\)` 或 `\[f(x)=\int_{0}^{x}e^{-f(t)}\,dt\]`）。为此，`ky-cli` 创新推出了\*\*“双层可视化与 LaTeX 还原方案”\*\*：

1. **第一层：终端控制台 Unicode 自动美化**
    - 终端输出推导后，内置的 `latex_beautifier` 技能会自动将积分号、导数、分式、上标下标与希腊字母还原为直观易懂的 Unicode 排版：
2. **第二层：浏览器实时可视化伴侣 (Live LaTeX Companion)**
    - 终端内随时输入 `/view`（或直接运行 `ky view`），立即在浏览器中弹出侧边**印刷级 KaTeX 实时排版伴侣窗口**（`docs/live.html`）；
    - **终端与 Web 伴侣两端实时同步**：无论在命令行还是在网页窗口敲字，微积分、行列式、高阶导数与采分点批改均以考研教材标准优雅呈现！
...
> 3. 交互式选择**私教辅导风格（严格/秒杀/鼓励/溯源）与主要使用的 Agent 工具**；
...
![每日数字化私教闭环 SOP](docs/assets/daily_study_loop.jpg)
...
![移动端自测看板与遮罩默写功能演示](docs/assets/dashboard_showcase.jpg)
