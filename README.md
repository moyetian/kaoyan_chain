...
### 5. 🏛️ KaoYan Intelligence · 高校招考情报与证据链引擎 (`ky admission` & `ky watch` & `ky scout`)
...
- **研招网 (S级) 与高校官网 (A级) 权威事实核验 (`ky admission`)**：
  - 执行 `ky admission <高校名> [专业代码/名] [--year=2027] [--save]`；
...
- **招生动态指纹监控雷达与晨报速递 (`ky watch` & `ky today`)**：
  ...
  - 每天早晨运行 `ky today` 或启动终端时，若监控雷达发现官方最新招生变动，**自动置顶弹出「🔥 研招动态突发情报速递」提示卡片**。
- **PDF 官方招生目录与简章内文深度解析**：
  - 内置基于二进制结构与 `pypdf` 的深度文档解析器，从高校发布的招生目录 PDF 附件中直接智能提取专业代码、拟招计划人数与初试统考 408 对应门类。
- **个人学情量化报考风险与提分门槛诊断 (User State Gap Analysis)**：
  - 智能联动学员在 `ky_config.json` 设定的初始摸底分与战役目标成绩，对标目标高校历史自划线与名校报录比，量化推算复试安全缓冲垫、数学与专业课最低提分红线。
- **社媒就读体验与避坑全景 (`ky scout`)**：
  - 执行 `ky scout <高校名> [专业名] [--save]`，聚合知乎（导师风评）、B站（高分复盘）、小红书（压分/避坑）实名讨论，出具全景研报。
