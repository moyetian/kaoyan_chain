...
| `/admission` | 研招网与高校官方招考事实与证据链核验 (S/A级权威) | 精准核实最新初试科目组合与专业代码 |
| `/watch` | 动态招生监控雷达 (添加/移除监控高校，比对最新简章变动) | 追踪高校研究生院官网最新公告与科目变更 |
| `/compare` | 双校招考核心指标横向深度对标 (408/自命题/复试线/保护) | 择校决赛圈两所目标高校横向对比 |
| `/scout` | 目标高校研招简章、自命题大纲、拟招人数与知乎/B站口碑侦察 | 择校探查、评估初试自命题难度与避坑 |
...
## 🏛️ KaoYan Intelligence · 高校招考全景情报与权威证据链中枢

本项目彻底摒弃了市面上通用 AI “胡乱搜索几个关键词扔给大模型瞎编”的玩具级做法，在核心底层打造了严谨的 **KaoYan Intelligence 六层招考情报与证据链引擎**：
...
### 1. 🏛️ 全国 55+ 所主流高校官方注册表与站点树 (`data/universities/registry.json`)

- **预置全国 55+ 研招院校名录**：收录 5 位教育部高校代码、别名简称（如“华科”、“南医大”、“成电”、“两电一邮”）、高校官网、研究生院及招生办二级域名；
- **有向站点树自动拓扑发现**：构建从 `高校官网 → 研究生院 → 招生网 → 二级学院招生专栏` 的完整站点拓扑，直接穿透到具体学院。

### 2. 📜 研招网 (S级) 与高校官网 (A级) 权威招考事实核验 (`ky admission`)

- **参数化官方直连**：执行 `ky admission <高校名> [专业代码/名] [--year=2027] [--save]`；
- **权威证据链对象**：输出带有信源分级（S/A/B/C/D）、置信度（100%/85%/30%）与官方发布时间戳的 `EvidenceObject`；
- **年份锁定机制 (Exam Year Locking)**：强行绑定目标考研年份（如 2027），严防将陈旧往年考情误判为当年新政；
- **多源冲突仲裁 (Conflict Resolver)**：当研招网基准与学院最新通知产生名额出入时，自动标记 `STATUS: CONFLICT` 并输出双源比对研判建议。
...
### 4. ⚖️ 双校招考核心指标深度横向对标决策引擎 (`ky compare`)

- **核心考情横向 PK**：执行 `ky compare <高校1> <高校2> [专业] [--save]`，深度横向对标两校的教育部代码、所在城市、办学层次、自划线特征、初试科目差异（如 408 统考 vs 自命题）、近三年复试线走势与**一志愿保护机制**；
- **研报自动导出**：自动输出结构化对比表格与私教深度择校处方，一键导出至 `04-专业课/` Markdown 研报。

<p align="center">
  <img src="docs/assets/school_comparator_matrix.svg" alt="ky compare 考研双校招考横向深度对标与决策矩阵" style="max-width:100%;" />
</p>

### 5. 📡 招生动态指纹监控雷达与晨报速递 (`ky watch` & `ky today`)

- **目标院校动态列管**：执行 `ky watch <高校名>` 将目标高校纳入动态监控雷达；
- **页面指纹毫秒比对**：执行 `ky watch --check` 自动比对网页标题与 SHA256 哈希指纹，每年 8~10 月第一时间捕获 2027 最新简章与自命题大纲变动；
- **晨起第一眼突发预警**：每天早晨运行 `ky today` 或启动终端时，若监控雷达发现官方最新招生变动，**自动置顶弹出「🔥 研招动态突发情报速递」提示卡片**。

### 6. 📑 官方 PDF 招生目录与简章内文深度智能解析

- **PDF 二进制流深度提取**：内置基于二进制结构与 `pypdf` 的深度文档解析器，高校发布的招生目录 PDF 附件无需转格式即可直接智能抽取专业代码、拟招计划人数与初试统考 408 对应门类。

### 7. 🎯 个人学情量化报考风险与提分门槛诊断 (User State Gap Analysis)

- **目标分与复试线联动**：智能联动学员在 `ky_config.json` 设定的初始摸底分与战役目标成绩，对标目标高校历史自划线与报录比，量化推算复试安全缓冲垫、数学与专业课最低提分红线。

### 8. 🔍 社媒就读体验口碑与避坑全景侦察 (`ky scout`)

- **三大社媒深度聚合**：执行 `ky scout <高校名> [专业名] [--save]`，聚合知乎（导师风评/实验室氛围）、B站（高分上岸学长复盘）、小红书（压分/复试差额比）实名讨论，出具立体全景研报。
...
============================================================
 [测试组 1]  配置文件与默认值校验 .................................... [PASS]
 [测试组 2]  四科私教提示词与上下文组装 (防书目幻觉) ................. [PASS]
 [测试组 3]  微信/QQ/钉钉/飞书 报文与未配置容错 ...................... [PASS]
 [测试组 4]  每日晨报与自测卡片自动提取 .............................. [PASS]
 [测试组 5]  Webhook 网关 HTTP 服务 (模拟微信/钉钉/QQ 呼入请求) ...... [PASS]
 [测试组 6]  本地自测看板编译集成校验 ................................ [PASS]
 [测试组 7]  Git 隐私隔离机制实测 .................................... [PASS]
 [测试组 8]  考研专有 Skills 体系功能实测 (SymPy/PDF抽取/视觉/启发) ... [PASS]
 [测试组 9]  考研科目方案与官方大纲管理体系校验 ...................... [PASS]
 [测试组 10] 个人定制化必考方案设计引擎与状态持久化校验 .............. [PASS]
 [测试组 11] 专有技能中枢重度升级与全流程闭环校验 .................... [PASS]
 [测试组 12] 工业级 Agent Loop、工具分发、权限安全与沙箱拦截全链路回归 [PASS]
 [测试组 13] 三级分层记忆、生命周期拦截钩子、网络搜索与 MCP 客户端回归 [PASS]
 [测试组 14] Agent 关键工具真实执行级校验 (防止签名漂移与隐藏崩溃) ... [PASS]
 [测试组 15] 上下文压缩配对保护 (防孤儿Tool) 与 CLI 子命令回归 ....... [PASS]
 [测试组 16] Sprint 2 教学闭环核心功能全量回归 (组卷/变式/图谱/诊断/减负) [PASS]
