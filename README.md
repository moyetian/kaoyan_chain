...
| **院校侦察** | `ky scout <高校名> [专业名] [--save] [--apply]` | 定向侦察目标高校研招简章、专业目录、自命题大纲、拟招人数、报录比，聚合知乎/B站/小红书就读体验与避坑指南 | 择校择专业调研、探查压分与复试公平度 |
...
### 5. 🎯 目标高校研招与社媒口碑侦察引擎 (`ky scout`)
- **官方研招直达**：执行 `ky scout <高校名> [专业名]`，自动定向穿透高校研究生院官网与研招网，秒级抓取最新招生简章、专业目录、自命题初试大纲、拟招人数与历年报录比；
- **社媒就读体验与避坑聚合**：定向汇总知乎（导师与实验室风评、专硕学硕待遇）、哔哩哔哩（高分备考经验贴、复试线走势）、小红书（是否压分、有无调剂歧视、复试差额比高低）的实名讨论；
- **AI 战略级深度研报**：若配置了大模型，AI 自动提炼【基本招考盘】+【竞争态势分析】+【网络舆情画像】+【避坑红黑榜】+【私教备考战术】五维深度研报；
- **备考档案闭环联动**：
  - 加 `--save` 参数：自动将完整研报落盘保存至 `04-专业课/目标院校情报_<高校>_<专业>.md`；
  - 加 `--apply` 参数：一键将目标高校、报考专业与识别到的初试科目（如 408/自命题代码）同步更新至 `ky_config.json` 与顶层备考协议中！
