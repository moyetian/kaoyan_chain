...
本项目包含全方位的自动化回归测试套件（覆盖沙箱越界拦截、考纲红线 Hook、三级记忆、MCP 客户端与真题抽题等），保障长期使用的高可用性：
...
└── tools/                               # 跨平台运维与管理工具包
    ├── agent/                           # 工业级自主智能体内核 (Agent Loop, Hooks, Memory, MCP, Sandbox)
    ├── skills/                          # 考研专有能力技能中枢 (SymPy验算, PDF抽题, 艾宾浩斯盲盒)
    ...
    ├── test_ky_suite.py                 # 108项全自动化工程测试套件
