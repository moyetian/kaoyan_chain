...
# [根因修复·日期硬编码] 初试日期改由 exam_calendar 统一推算，替代此前写死的
# f"{year}-12-19"（year 取 now.year + 1，会把 2027 届考生的初试日写晚整整一年）
try:
...
def configure_profile(interactive=True, math_key="math2", eng_key="eng2", pro_type="custom", pro_name="专业课"):
    ...
    if interactive:
        ...
        year = input(
            f"  3. 考研年份(入学年，如 2027 表示 2026 年 12 月初试) [默认: {datetime.now().year + 1}]: "
        ).strip() or str(datetime.now().year + 1)
    ...
    # [根因修复·日期硬编码] 旧实现写 f"{year}-12-19"：year 为「考研年份（入学年）」，
    # 其初试在头一年 12 月，直接用 year 会整体写晚一年（2027 届写成 2027-12-19），
    # 且末尾的"(倒计时约 N 天)"后缀被整行替换后彻底丢失，只能等 study_planner 再补。
    exam_date_obj = None
    if exam_calendar is not None:
        try:
            exam_date_obj = exam_calendar.exam_date_for_enrollment_year(int(str(year).strip()[:4]))
        except Exception:
            exam_date_obj = exam_calendar.resolve_exam_date({})[0]
    if exam_date_obj is None:
        exam_date_obj = datetime.now().date()
    days_left = max(0, (exam_date_obj - datetime.now().date()).days)
    content = re.sub(
        r"- \*\*初试日期\*\*：.*",
        f"- **初试日期**：`{exam_date_obj.isoformat()}` (倒计时约 {days_left} 天)",
        content,
    )
