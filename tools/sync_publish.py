...
SRC = Path(__file__).resolve().parent.parent          # 默认：本仓库工作区根
DST = Path(os.environ.get("KAoyan_PUBLISH_DST",
                          str(SRC.parent / "kaoyan_chain_public")))  # 发布副本输出目录（可用环境变量覆盖）
...
SUBSTITUTIONS = [
    ...
    # [安全] 密钥脱敏改为通用正则：任何形态的 API Key 都不允许进入公开副本。
    # （历史版本在此硬编码了真实 Key 字面量，已随 v2.7 清理并重建历史。）
    (r"sk-[A-Za-z0-9]{20,}", "YOUR_API_KEY_HERE"),
    (r"gpt-5\.4-mini", "gpt-4o-mini"),
    # 联系方式（QQ / 邮箱）按学员要求随仓库公开，不再做脱敏映射。
