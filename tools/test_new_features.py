...
def run_new_feature_tests(strict: bool = False):
    ...
    try:
        ...
        # A.3 搜索本地降级与备用结果
        exp_dir = Path("tools").resolve().parent / ".memory" / "experiences"
        exp_dir.mkdir(parents=True, exist_ok=True)
        dummy_file = exp_dir / "dummy_计算机_test.md"
        dummy_file.write_text("# 华科计算机经验\n\n**来源公众号**: CS考研圈  \n**发布日期**: 2026-09-14  \n**原文链接**: [http://test](http://test)  \n\n测试", encoding="utf-8")
        try:
...
"""
            ...
            # B.4b 科目抽取双模一致性 —— accel 黑名单解除后的回归防线。
            # 历史缺陷：Rust 版把 "(204)英语(二)" 截断成 "(204)英语"，该错误条目同样
            # 占用 4 个名额，把真正的自命题科目 (814) 挤出结果。extractor.rs 已修复并
            # 重新编译；此处锁定两条路径的语义一致性（比较科目代码序列而非文案细节）。
            subj_sample = "一、初试科目：\n(101)思想政治理论\n(204)英语(二)\n(302)数学(二)\n(814)通信原理"
            if hasattr(rust_mod, "extract_subjects"):
                import os as _os
                import re as _re
                from tools import accel as _accel_mod
                ...
                rust_subjs = rust_mod.extract_subjects(subj_sample)
                # 强制走纯 Python 路径做对照（黑名单解除后 _extract_subjects 会路由到 Rust）
                old_env = _os.environ.get("KAOYAN_DISABLE_ACCEL")
                _os.environ["KAOYAN_DISABLE_ACCEL"] = "1"
                _accel_mod.reset_cache()
                try:
                    py_subjs = _extract_subjects(subj_sample)
                finally:
                    if old_env is None:
                        _os.environ.pop("KAOYAN_DISABLE_ACCEL", None)
                    else:
                        _os.environ["KAOYAN_DISABLE_ACCEL"] = old_env
                    _accel_mod.reset_cache()
                runner.assert_true(
                    _codes(rust_subjs) == _codes(py_subjs) == ["101", "204", "302", "814"],
                    "Rust vs Python: extract_subjects 科目代码序列一致 (101/204/302/814)")
                runner.assert_true(
                    any(("英语(二)" in s or "英语（二）" in s) for s in rust_subjs),
                    "Rust extract_subjects: (204)英语(二) 不再被截断")
                runner.assert_true(
                    any("814" in s for s in rust_subjs),
                    "Rust extract_subjects: 自命题 (814) 不再被挤出名额")
