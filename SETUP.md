# 考研学习链 (Kaoyan AI Study Chain) · 部署与进阶工程配置手册

> **【系统最高目标】** 本文档为考研学习链的**工程级部署与进阶技术配置手册**。
> 💡 若你是考研学子、寻求日常刷题、打卡与快速备考指令，请直接参阅更通俗易懂的：👉 **[【📘 学员实操与备考通关手册 (操作手册.md)】](操作手册.md)**。
...
- [4. 靶向组卷、考纲Diff、切片入库与变式实战](#4-靶向组卷考纲diff切片入库与变式实战)
...
- **Python 运行时**：Python 3.8+（推荐 Python 3.10+）；基础 CLI 使用标准库，数学/PDF/图像/OCR 技能按需安装 `requirements.txt` 中的可选依赖。
- **Git**：用于版本管理与多设备同步
...
1. **晨间（确认与广播）**：
   ...
   - 配置对应平台机器人后，运行 `ky notify` 将任务广播推送到微信、钉钉或 QQ 备考群；
2. **日间（攻坚与采分）**：
...
## 4. 靶向组卷、考纲Diff、切片入库与变式实战

### 4.1 靶向自测组卷 (`ky compose` / `ky exam`)
...
```
- 试卷自动抹去你之前的所有错误记录与解题线索，生成标准答题卡格式；
- 试卷末尾嵌入了加密的采分点元数据（`EXAM_ANSWER_KEYS`）；
- 独立完成后，提交批改：
```bash
...
### 4.3 官方考纲 AST 版本比对与动荡率分析 (`ky fetch`)
当每年 8~9 月教育部或院校公布新大纲时，一键锁定所有变动细节：
```bash
# 比对本地基准大纲与新版大纲
...
- **四维状态标色判定**：
  - `[新增]`：新大纲新增知识点（如泰勒公式展开）；
  - `[删除]`：被剔除的低频知识点；
  - `[要求提升]`：如零点定理从“理解”提升为“掌握”；
  - `[要求降低]`：如从“掌握”降级为“了解”；
- **波动率量化评级**：输出动荡率（如 `10.7%`），并针对性出具增补与减负复习处方。

---

### 4.4 外部真题/模拟卷智能切片入库 (`ky ingest`)
将散落的历年真题 Markdown 文件、高质量模拟题结构化入库至本地题库：
```bash
ky ingest "参考资料/2024年408模拟精选题.md" --subject=专业课 --save
```
- **智能分块算法**：自动识别选择题（精准提取 A/B/C/D 选项）与解答题；
- **采分点解析**：自动抽取解答题的分步赋分（如 `[+6分]` / `[+9分]`）；
- **标准化归档**：切片后自动存入对应科目的 `参考资料/`，供私教日常精准抽题与靶向组卷。

---

### 4.5 官方考纲树状图谱与掌握度 (`ky map`)
```bash
...
### 4.7 高校招考证据核验与简章监控 (`ky admission` & `ky watch` & `ky scout` & `ky compare`)
...
- **学情量化差距诊断**：联动学员 `ky_config.json` 目标分数，量化研判复试安全缓冲垫与单科提分死命令；
...
### 5.1 微信个人号 (WeChat ClawBot 扫码直连 · 需第三方连接器)
- **前置条件**：需要单独安装并授权 WeChat ClawBot 连接器；本项目只提供启动入口和本地 API 网关。
- **操作步骤**：
  1. 在终端直接运行：
     ```bash
     ky clawbot
     ```
  2. 连接器按其文档输出登录二维码，打开手机微信【扫一扫】授权登录；
...
4. **隐私脱敏提示**：如部署至公开仓库，请在本地运行：
   ```bash
   python tools/update_dashboard.py --local
   ```
...
### 7.2 自动化回归测试 (`python tools/test_ky_suite.py`)
```bash
python tools/test_ky_suite.py
```
涵盖模块单元测试、模拟服务集成测试和 CLI 真实进程级 smoke test；测试数量以运行时输出为准：
1. 配置文件与容错机制
